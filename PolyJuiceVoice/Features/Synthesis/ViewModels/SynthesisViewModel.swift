...
final class SynthesisViewModel: ObservableObject {
    ...
    /// When non-nil, the user hasn't configured a model for the capability
    /// the current voice selection needs. View shows
    /// `MissingCapabilityPrompt(capability:)`.
    ...
    func setup(ttsService: MLXTTSService,
        ...
        // `unconfiguredCapability` (cheap disk check) so the UI can render
        // the prompt right away. The actual model load is deferred until
        // `synthesize()` runs — see comment there for why.
        $selectedOption
            .removeDuplicates()
    ...
    /// Cheap, synchronous: checks whether the user has a selected + installed
    /// snapshot for the capability the current voice selection needs.
    private func refreshUnconfiguredCapability() {
        ...
        if let snap = store.selected(for: needed),
           ModelDownloadManager.isInstalled(snap) {
            unconfiguredCapability = nil
        } else {
            unconfiguredCapability = needed
        }
    }
    ...
    private func loadCapabilityForSelection() async {
        ...
        let needed = selectedOption.requiredCapability
        do {
            try await tts.loadCapability(needed, using: store)
            unconfiguredCapability = nil
