...
echo "🔍 Checking PolyJuiceVoice app bundle size..."
echo ""

# Find the most recent build
APP_PATH=$(find ~/Library/Developer/Xcode/DerivedData -name "PolyJuiceVoice.app" -path "*Debug-iphoneos*" -type d 2>/dev/null | head -1)

if [ -z "$APP_PATH" ]; then
    echo "⚠️  No app bundle found. Please build the project first:"
    echo "   xcodebuild build -project PolyJuiceVoice.xcodeproj -scheme PolyJuiceVoice -destination 'generic/platform=iOS'"
    exit 1
fi
...
if [ -z "$LARGE_FILES" ]; then
    ...
    echo "      rm -rf ~/Library/Developer/Xcode/DerivedData/PolyJuiceVoice-*"
    echo "      xcodebuild build -project PolyJuiceVoice.xcodeproj -scheme PolyJuiceVoice -destination 'generic/platform=iOS'"
    echo ""
    echo "   2. Move model files outside project (recommended):"
    echo "      mkdir -p ~/Developer/MLXModels"
    echo "      mv PolyJuiceVoice/Resources/MLXModels/* ~/Developer/MLXModels/"
